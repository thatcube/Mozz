...
let package = Package(
    ...
    targets: [
        ...
        .testTarget(
            ...
            linkerSettings: [
                .unsafeFlags([
                    "-Laudio/target/swift", "-lmozz_audio_ffi",
                ]),
                // A Rust staticlib does not carry the system frameworks it
                // needs, so the linking side has to name them. These are what
                // cpal reaches CoreAudio through; the equivalents on Windows
                // and Android are named by whatever links there.
