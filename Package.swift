...
// Linking the shared audio engine.
//
// SwiftPM cannot run cargo, so `tools/build-audio-staticlib.sh` produces this
// library first and any *product* that reaches the engine names it here.
// Library targets do not link - only executables and test bundles do - so this
// is needed on test targets that transitively depend on MozzAudioEngine, and
// nowhere else. That was verified rather than assumed: a library target with
// the dependency builds fine without these flags, and a test target without
// them fails with undefined symbols.
//
// Defined once because the alternative is the same four lines copied onto
// every test target that happens to reach playback, and the copy that gets
// forgotten fails at link time on someone else's machine.
//
...
let package = Package(
    ...
    targets: [
        ...
        .target(name: "MozzPlayback", dependencies: ["MozzCore", "MozzAudioEngine"]),

        // MARK: The shared audio engine
        //
        // CMozzAudio publishes the generated C header for the Rust engine; it
        // has no implementation of its own and must never grow one, because a
        // function defined on this side would be one the shared engine does not
        // have. The header is generated by tools/generate-audio-header.sh and
        // CI fails if the committed copy has drifted, since a mismatched
        // signature is a compile error on neither side - it is a corrupted
        // argument at run time, inside an audio callback.
        //
        ...
        .target(
            ...
            dependencies: [
                ...
                "MozzAnalysis", "MozzPairing", "MozzRelay",
            ],
        ...
        .target(
            ...
            dependencies: [
                ...
                // Pairing. The protocol and the crypto live here so a non-Apple
        ...
        // needs the real staticlib. It keeps a missing library from breaking
